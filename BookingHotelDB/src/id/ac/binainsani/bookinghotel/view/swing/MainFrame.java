...
public class MainFrame extends JFrame {
    ...
    private JPanel createBookingPanel() {
        ...
        List<HotelRoom> availableRooms = new java.util.ArrayList<>(roomService.getAllRooms()
                .stream().filter(HotelRoom::isAvailable).toList());
        ...
        refreshButton.addActionListener(e -> {
            ...
            List<HotelRoom> newRooms = roomService.getAllRooms().stream().filter(HotelRoom::isAvailable).toList();
            availableRooms.clear();
            availableRooms.addAll(newRooms);
            for (HotelRoom room : newRooms) {
                roomCombo.addItem(room.getRoomNumber() + " - " + room.getType());
            }
            updatePrice(roomCombo, durationSpinner, priceField, availableRooms);
        });
    ...
    private JPanel createAdminControlsPanel() {
        ...
        JComboBox<String> typeCombo = new JComboBox<>(new String[]{"single", "double"});
    ...
    private void showAboutDialog() {
        JOptionPane.showMessageDialog(this,
                ...
                + "Version 1.0\n"
                + "Developed by 'OOP jaya jaya jaya'",
                "About",
                JOptionPane.INFORMATION_MESSAGE);
    }
